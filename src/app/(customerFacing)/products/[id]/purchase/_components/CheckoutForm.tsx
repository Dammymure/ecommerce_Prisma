...
function Form({ priceInCents, productId} : { priceInCents: number, productId: string}) {
    ...
    async function handleSubmit(e: FormEvent){
        ...
        if (orderExist) {
            setErrorMessage("You have already purchased this order, try downloading it from the My Orders page")
            setIsLoading(false)
            return
        }
        ...
        <Card>
            ...
            <CardFooter>
                <Button className="w-full" size="lg" disabled={stripe == null || elements == null || isLoading}>
                    {isLoading ? "Purchasing..." : `Purchasing - ${formatCurrency(priceInCents / 100)}`}
                </Button>
            </CardFooter>
        </Card>
